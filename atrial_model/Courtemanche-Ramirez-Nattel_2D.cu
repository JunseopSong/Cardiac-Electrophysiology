...
#define g_K1  0.09*2.0   // nanoS_per_picoF (in time_independent_potassium_current)
#define g_to  0.1652*0.3   // nanoS_per_picoF (in transient_outward_K_current)
#define g_Kr  0.029411765*1.6   // nanoS_per_picoF (in rapid_delayed_rectifier_K_current)
#define g_Ca_L  0.12375*0.5   // nanoS_per_picoF (in L_type_Ca_channel)
...
#define M 512
#define DiffCoeff 0.001 // cm^2/ms
#define dx_ 0.025 // cm
// ------------------------------ PROGRAM SETTING END --------------------------------
...
#define dt 0.05
#define itmax 300100 // time limit
#define tol 0.000001

double pacingStartTime[] = {0, 500};
...
#define expdt_tau_u 0.993769491
#define expdt_tau_f_Ca 0.975309912
// --------------------------------------------------
...
__global__ void diffKernel(double *volt_after, double *volt_before, const bool *nonExcitable, const int num)
{
	...
	double Dfudtdx2 = DiffCoeff*dt/(dx_*dx_)/2.0;
	...
	for(int id=bid * bdim + tid; id<num; id+=step)
	{
		...
		if(x-1 >= 0)
		{
			volt_after[id] += Dfudtdx2*(volt_before[id-M] - volt);
		}
		if(x+1 <M)
		{
			volt_after[id] += Dfudtdx2*(volt_before[id+M] - volt);
		...
		if(y-1 >= 0)
		{
			volt_after[id] += Dfudtdx2*(volt_before[id-1] - volt);
		}
		if(y+1 <M)
		{
			volt_after[id] += Dfudtdx2*(volt_before[id+1] - volt);
...
int main()
{
	...
	for(iter=1; iter<=itmax; iter++)
	{
		...
		if(!S2Finish)
		{
			...
			if(S2Check && S1S2targetvolt < -55.0)
			{
				S2Finish = true;
				S2Stim = true;
				S2_stim_start_ttt = ttt;
				pacingFinish = true;

				for(i=0; i<M; i++)
				{
					for(j=M/2; j<M; j++) Istim[index(i,j)] = true;
				}
				cudaMemcpy(stimulation, Istim, sizeof(bool)*nde, cudaMemcpyHostToDevice);
			}
		}

		if(S2Stim && ttt >= S2_stim_start_ttt+1.0)
		{
			S2Stim = false;
			for(i=0; i<nde; i++) Istim[i] = false;
			cudaMemcpy(stimulation, Istim, sizeof(bool)*nde, cudaMemcpyHostToDevice);
		...
		diffKernel<<<grid, threads>>>(volt_d_, volt_d, ifscar_d, nde);

		diffKernel<<<grid, threads>>>(volt_d, volt_d_, ifscar_d, nde);
